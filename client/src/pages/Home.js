...
const [inputSoal,setInputSoal]=useState('');
  ...
  const res=await axios.post("https://backend-pembangkit-soal.vercel.app",{nomor:e.target.parentNode.nextElementSibling.value,inputSoal,jumlahSoal});
...
const hoverTooltip=(e)=>{
  ...
  for(let i=1;i<11;i++){
      ...
      <label className="text-l font-serif font-semibold tracking-wider uppercase text-[#2B2E4A]" htmlFor={'soal'+i}>Tujuan Pem {i}</label>
